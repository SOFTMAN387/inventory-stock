...
// import { UserToken } from '../../utils/authToken';
...
console.log(productData);
  ...
  const submitProduct=async()=>{
    try {
      ...
      if(!fileData.url){
        ...
        const res=await axios.post(`${BASE_URL}/api/product/new-product`,
  ...
  <h1 className="font-medium text-xl">Add New Product</h1>
  <br/>
    <div className="mt-8 grid lg:grid-cols-2 gap-4">
      <div>
        <label for="title" className="text-sm text-gray-700 block mb-1 font-medium">Title</label>
        <input type="text" name="title" onChange={handleInputChange} id="title" className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Enter Title" />
      </div>

      <div>
        <label for="price" className="text-sm text-gray-700 block mb-1 font-medium">Price</label>
        <input type="number" name="price" onChange={handleInputChange} id="price" className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Enter Price" />
      </div>

      <div>
        <label for="quantity" className="text-sm text-gray-700 block mb-1 font-medium">Quantity</label>
        <input type="number" name="quantity" onChange={handleInputChange} id="quantity" className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Enter Qyantity" />
      </div>

      <div>
        <label for="rating" className="text-sm text-gray-700 block mb-1 font-medium">Rating</label>
        <input type="number" name="rating" onChange={handleInputChange} id="rating" className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Enter Rating" />
      </div>
    </div><br/>
    <div>
    ...
    <div>
        <label for="sub_category" className="text-sm text-gray-700 block mb-1 font-medium">Sub Category</label>
        <input type="text" name="sub_category" onChange={handleInputChange} id="sub_category" className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Enter Sub Category" />
      </div>
      <br/>
      <div className="relative w-full min-w-[200px]">
    <textarea
     name='description' onChange={handleInputChange} className="peer h-full min-h-[100px] w-full resize-none rounded-[7px] border border-teal-100 border-t-transparent bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-green-500 placeholder-shown:border-t-green-500 focus:border-2 focus:border-green-500 focus:border-t-transparent focus:outline-0 disabled:resize-none disabled:border-0 disabled:bg-blue-gray-50"
  ...
  {inputErr&&<span className='text-red-500'>{inputErr}</span>}
    <div className="space-x-4 mt-8">
      <button onClick={submitProduct} className="py-2 px-4 bg-teal-500 text-white rounded hover:bg-teal-600 active:bg-blue-700 disabled:opacity-50">Submit</button>

      <button className="py-2 px-4 bg-red-500 border border-gray-200 text-white rounded hover:bg-red-700 active:bg-gray-200 disabled:opacity-50" onClick={()=>navigate("/admin/products")}>Cancel</button>
    </div>
</div>
